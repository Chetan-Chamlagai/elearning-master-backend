...
//    List<Payment> findPaymentsByDateRange(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);
    ...
    @Query("SELECT p FROM Payment p JOIN p.categories c WHERE p.user.id = :userId AND c.categoryId IN :categoryIds AND p.status = 'PENDING'")
    List<Payment> findPendingPaymentsByUserIdAndCategoryIds(@Param("userId") Integer userId, @Param("categoryIds") List<Integer> categoryIds);
